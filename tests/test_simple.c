...
main(void)
{
	...
	/* --- lookup -------------------------------------------------------- */
	check("finds root by name",          msl_simple_find("root") != NULL);
	check("finds root by path",          msl_simple_find("/root") != NULL);
	check("name and path agree",         msl_simple_find("root") == msl_simple_find("/root"));
	check("finds run",                   msl_simple_find("run") != NULL);
	check("finds srv",                   msl_simple_find("srv") != NULL);
	check("rejects an unknown name",     msl_simple_find("nonesuch") == NULL);
	check("rejects empty",               msl_simple_find("") == NULL);
	check("rejects NULL",                msl_simple_find(NULL) == NULL);

	/* The components with real work behind them are not simple nodes. */
	check("home is not a simple node",   msl_simple_find("home") == NULL);
	check("mnt is not a simple node",    msl_simple_find("mnt") == NULL);
	check("media is not a simple node",  msl_simple_find("media") == NULL);

	/* --- the table ----------------------------------------------------- */
	check("three nodes defined",         msl_simple_node_count == 3);

	for (size_t i = 0; i < msl_simple_node_count; i++) {
		const struct msl_simple_def *d = &msl_simple_nodes[i];
		char desc[128];
		...
		check(desc, d->state != NULL && strncmp(d->state, "msl.", 4) == 0);
		...
		check(desc, !d->creates_target ||
		    strncmp(d->target, MSL_DATA_ROOT "/", sizeof(MSL_DATA_ROOT)) == 0);
	}

	/* --- the specific mappings, which are the point of the component ---- */
	check("/root points at /var/root",
	    strcmp(msl_simple_find("root")->target, "/var/root") == 0);
	check("/run points at /var/run",
	    strcmp(msl_simple_find("run")->target, "/var/run") == 0);
	check("/root does not create its target",
	    !msl_simple_find("root")->creates_target);
	check("/run does not create its target",
	    !msl_simple_find("run")->creates_target);
	check("/srv does create its target",
	    msl_simple_find("srv")->creates_target);

	/* --- status against the real system, read-only ---------------------- */
	check("status of /root reads",       msl_simple_status(msl_simple_find("root"), &st) == 0);
	check("/var/root exists, so /root's target is present", !st.target_missing);
	check("status of /run reads",        msl_simple_status(msl_simple_find("run"), &st) == 0);
	check("/var/run exists, so /run's target is present",   !st.target_missing);

	check("status rejects NULL def",     msl_simple_status(NULL, &st) == -1);
