...
{
	if (fhs_write_atomic(SYNTHETIC_CONF, content, strlen(content), 0644) != 0) {
...
get(void)
{
	char *s = fhs_slurp(SYNTHETIC_CONF, NULL);
	if (s == NULL) {
...
main(void)
{
	 ...
	 * fhs_skeleton_add(), which requires root and creates directories on the
	...
	put("proc\nmnt\t" FHS_DATA_ROOT "/mnt\nmedia\t" FHS_DATA_ROOT "/media\n");
	check("remove reports a change",            fhs_skeleton_remove("mnt") == 1);
	check("our entry is gone",                  !has("mnt\t"));
	check("procfs's entry survives",            has("proc\n"));
	check("the media entry survives",           has("media\t" FHS_DATA_ROOT "/media"));
	check("remove is idempotent",               fhs_skeleton_remove("mnt") == 0);

	/* --- removal must not touch a same-named entry we did not write ---- */
	put("mnt\t/somewhere/else\n");
	before = get();
	check("a foreign entry is not removed",     fhs_skeleton_remove("mnt") == 0);
	after = get();
	...
	put("mnt\n");
	check("a bare-name entry is not removed",   fhs_skeleton_remove("mnt") == 0);
	check("the bare entry survives",            has("mnt"));

	/* --- comments and blank lines survive a rewrite -------------------- */
	put("# a comment\n\nproc\nmnt\t" FHS_DATA_ROOT "/mnt\n");
	check("remove succeeds with comments present", fhs_skeleton_remove("mnt") == 1);
